...
describe("bulk roll", () => {
  ...
  it("disables the button while the rolls it just requested are still in flight", async () => {
    // init stays 0 for the whole duration of a roll — it is only written back
    // on success — so an unrolled-count predicate alone leaves the button
    // enabled and still reading "(2)" mid-flight. A GM watching dicex's
    // several-second animation can easily click it again, which would submit
    // the same tokens a second time and race two writes per token.
    __testHooks.setItems([token("gob-a", "gm-1"), token("gob-b", "gm-1")]);
    await mount(root);
    root.querySelector<HTMLButtonElement>("#fh-bulk")!.click();
    const bulk = root.querySelector<HTMLButtonElement>("#fh-bulk")!;
    expect(bulk.disabled).toBe(true);
    expect(bulk.textContent).toContain("(0)");
  });

  it("excludes an already-rolling combatant from a later bulk click", async () => {
    // Pins main.ts's copy of the predicate the same way the clamped-combatant
    // test above does: the button stays legitimately enabled here (gob-b is
    // idle), so the emitted rolls — not the disabled attribute — are what
    // prove the in-flight token was skipped.
    __testHooks.setItems([token("gob-a", "gm-1"), token("gob-b", "gm-1")]);
    await mount(root);
    root
      .querySelector<HTMLButtonElement>('.fh-roll[data-id="gob-a"]')!
      .click();
    ...
    bulk.click();
    const rolls = (
      __testHooks.broadcasts
        .filter((b) => b.channel === INTERNAL_ROLL_CHANNEL)
        .pop()!.data as { rolls: RollSpec[] }
    ).rolls;
    expect(rolls.map((r) => r.itemId)).toEqual(["gob-b"]);
  });
